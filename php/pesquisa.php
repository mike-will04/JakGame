...
<body>
    ...
    <?php
    $jogospesquisa = $conn->prepare('SELECT foto, tag, F.id_jogo
    FROM fotos F
    INNER JOIN tags T ON F.id_jogo = T.id_jogo
    WHERE tag = :pesquisa');
    $jogospesquisa->execute(array(
        ':pesquisa' => $pesquisa
    ));

    if ($jogospesquisa->rowCount() >= 0) {
        echo "<div class='divpai'>";
        foreach ($jogospesquisa as $linha) {
            echo "<div class='divjogos'><a href='../php/jogo.php?id=" . $linha['id_jogo'] . "'><img src='../img/admin/" . $linha['foto'] . "' style='width:150px; height:150px; border-radius: 15px; border: 2px solid black'></a></div>";
        }
        echo "</div>";
    } else {
